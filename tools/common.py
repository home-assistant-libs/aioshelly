# Common tools class
"""Class for aioshelly cmdline tools."""
...
class CommonTools:
    """Common method for building tools."""
    ...
    async def create_device(
        ...
        if gen in BLOCK_GENERATIONS:
            return await BlockDevice.create(
    ...
    async def connect_and_print_device(
        ...
        device = await self.create_device(aiohttp_session, options, init, gen)
        self.print_device(device)
        device.subscribe_updates(self.device_updated)
    ...
    def device_updated(
        ...
        try:
            self.print_device(cb_device)
        except InvalidAuthError:
            print("Invalid or missing authorization (from async init)")

    def print_device(self, device: BlockDevice | RpcDevice) -> None:
        """Print device data."""
        ...
        if device.gen in BLOCK_GENERATIONS:
            self.print_block_device(cast(BlockDevice, device))
        elif device.gen in RPC_GENERATIONS:
            self.print_rpc_device(cast(RpcDevice, device))

    def print_block_device(self, device: BlockDevice) -> None:
    ...
    def print_rpc_device(self, device: RpcDevice) -> None:
    ...
    async def update_outbound_ws(
        self, options: ConnectionOptions, init: bool, ws_url: str
    ) -> None:
        """Update outbound WebSocket URL (Gen2/3)."""
        async with aiohttp.ClientSession() as aiohttp_session:
            device: RpcDevice = await self.create_device(
                aiohttp_session, options, init, 2
            )
